---
name: xcheck-audit
description: Hand the next queued xcheck audit pass to the orchestrator - preflight, routing and the report `xcheck next` comes back with. Use when the user invokes $xcheck-audit or /xcheck-audit, says "run the audit" or "audit this project", or wants the next pass in audit/state.json executed.
---

# xcheck launcher — Auditor (wrapper)
...
1. **Preflight.** `audit/XCHECK.md` must exist in the current project — missing means xcheck is not installed here, so stop and point the person at the xcheck repository's `bootstrap.md` / `install.sh`. `audit/state.json` must exist too — without it the plan has not been migrated, and `xcheck migrate` is an operator act, not this launcher's. An operator profile must resolve as well: `xcheck status` prints the profile it is reading and its digest, and `xcheck next` refuses with no profile rather than falling back to defaults.
2. **Confirm this is the launcher for the work that is next.** Run `xcheck status`. It prints the decision the orchestrator would take, and it writes nothing. If that decision is not `run-auditor`, say what it is instead, name the launcher that fits it, and STOP — a Auditor launcher that dispatches something else is a launcher nobody can trust. `stop-triage`, `stop-critical` and `stop-embargo` are the gates that stop new passes on purpose: report the gate and what clears it, never work around it.
3. **Hand off.** Run `xcheck next`. It acquires the writing lock, builds the Auditor's charter from `audit/state.json`, dispatches the session under the operator's containment, couriers the result back and releases the lock. Do not do any of that yourself, and do not run the role's own verbs on its behalf: everything the session is allowed to write, it writes.
...
- **The improvised charter.** A charter typed at the launcher is gone. The orchestrator
  ...
  state with a verb —
  `xcheck queue-pass`, `xcheck amend-pass`, `xcheck cancel-pass` — and those are the operator's own commands, run at the
