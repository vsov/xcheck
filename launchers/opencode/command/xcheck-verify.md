---
description: Launch an xcheck Verifier session - give binding verdicts (closed/reopened) on fixed findings.
---
...
2. Read `audit/XCHECK.md` fully, then `audit/AUDIT.md` (charter limits — `reopen_limit` and any per-project overrides, §10). Your role: **Verifier** (§3 role card governs you). **Hard rule: you must not be the agent or session that produced these fixes.** Prefer a different agent than the one that ran remediation (cross-agent independence); the minimum acceptable fallback is a different, fresh session of the **same** agent (§3). Say which case applies before proceeding.
